...
def move_files(dir32, dir64, dirout):
    for filename in os.listdir(dir32):
        ...
        if os.path.islink(full_path):
            ...
            if os.path.islink(file64):
                try_mkdir_p(dirout)
                shutil.move(full_path, dirout)

        elif os.path.isfile(full_path):
            file64 = os.path.join(dir64, filename)
            if os.path.isfile(file64):
                try_mkdir_p(dirout)
                shutil.move(full_path, dirout)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', help="Output directory for 32-bit specific files", required=True)
    args = parser.parse_args()

    # After the script is finished, the directories will contain:
    # out:   32-bit specific files
    # dir32: files commonly used by 32-bit and 64-bit
